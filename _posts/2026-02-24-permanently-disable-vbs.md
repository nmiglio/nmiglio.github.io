...
Yep, you read it. Are you OK with that? I was not. In addition I need to run VirtualBox to spin a few virtual machines. When VBS is enabled le prestazioni della virtual machine vengono strozzate. And I wanted to squeeze all the computational power available. The risk should not be so high if you know what you are doing.. You decide for yourself, but I wanted out. So, how do I disable it? Should be an easy toggle to switch somewhere.. Well here is where the fun begins.

### There is no simple toggle to switch

Alright, so I decided to reclaim those precious frames per second. You'd think in this day and age, there'd be a big, shiny "Go Faster" button in the Windows settings. You're partially right. My first stop on this adventure was the "Core Isolation" page in Windows Security settings (search for it in the Start Menu). In there, there is a toggle for "Memory Integrity." I switched it off. "Ha!" you think, "That was easy!" A quick reboot with a smug grin on my face..

### Opting out is not that easy

After rebooting, I checked if it worked. Command Prompt. I typed `systeminfo`. I looked at the "Virtualization-based security" line. **"Running"**. At this point, I almost took the `regedit` way but in my experience it is always a descend into a frustrating rabbit hole of editing obscure keys you don't really know what are doing. You'll find guides telling you to hunt down keys like `EnableVirtualizationBasedSecurity` under `DeviceGuard`.
...
This script was the final word on the matter. It does everything the right way. I run it with the `-Disable` flag. It cleaned up not only all the necessary registry keys automatically (so you I didn't have to), but it also performrd some pre-boot magic. Using the Boot Configuration Data (`bcdedit`) tool it scheduled a special task for the next boot telling my computer: "Hey, before you even load Windows next time, run this special program (`SecConfig.efi`) and turn off VBS at the firmware level." This was it.
...
Then the true moment of truth. Once back in Windows, I opened a Command Prompt and typed `systeminfo` once again. I scrolled down the list and found the line for "Virtualization-based security". And there I saw the confirmation I won the battle this time: I could read the two most beautiful words in the English language: "Not enabled".

### Done

And this was it.I successfully navigated the labyrinth of Windows settings, bypassed the registry trap, and used the crushing tool to disable VBS for good. Now I can go enjoy the extra horsepower and smoother performance. I think I earned them.
